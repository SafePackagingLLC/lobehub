...
const AskAgentCommands = memo(() => {
  ...
  // Check if default assistant matches the query
  const showBridgePointAI = !mentionQuery || 'bridgepoint ai'.includes(mentionQuery);

  return (
    <Command.Group heading={t('cmdk.mentionAgent')}>
      {showBridgePointAI && (
        <Command.Item
          value="@bridgepoint-ai"
          onMouseDown={preventDefault}
          onSelect={() => handleAgentSelect(inboxAgentId, 'BridgePoint AI', DEFAULT_INBOX_AVATAR)}
        >
          <Avatar emojiScaleWithBackground avatar={DEFAULT_INBOX_AVATAR} shape="square" size={18} />
          <div className={styles.itemContent}>
            <div className={styles.itemLabel}>@BridgePoint AI</div>
          </div>
        </Command.Item>
      )}
